...
const ProductDetails = ({
  ...
  const addToCart = ({ emptyCard }: { emptyCard?: boolean }) => {
    addProductToCart({ product: { ...product, quantity }, emptyCard });
    setIsCartOpen(true);
  };
  const handleAddToCartClick = () => {
    ...
    if (hasDifferentRestaurantProduct) {
      return setIsConfirmationDialog(true);
    }
  ...
  return (
    <>
      <div className="relative mt-[-1.5rem] rounded-tl-3xl rounded-tr-3xl bg-white py-5">
        ...
        <div className="px-5">
          <DeliveryInfo
            restaurant={JSON.parse(JSON.stringify(product.restaurant, null, 2))}
          />
        </div>
      ...
      <AlertDialog
        ...
        onOpenChange={setIsConfirmationDialog}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Você tem certeza?</AlertDialogTitle>
            <AlertDialogDescription>
              Essa ação pode fazer com que seu carrinho seja esvaziado.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
